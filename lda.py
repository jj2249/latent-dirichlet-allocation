...
def LDA(A, B, K, alpha, gamma):
    ...
    num_gibbs_iters = 50
    thetas = np.zeros((K, num_gibbs_iters))
    entropies = np.zeros((K, num_gibbs_iters))
    for iter in tqdm(range(num_gibbs_iters)):
        thetas[:, iter] = (sk.flatten()+alpha)/np.sum(sk.flatten()+alpha)
        betas = (swk + gamma)/np.sum(swk+gamma, axis=0)
        entropies[:,iter] = np.nansum(-betas*np.log2(betas), axis=0)
        for d in range(D):
    ...
    return perplexity, swk, thetas, entropies
...
if __name__ == '__main__':
    ...
    perplexity, swk, thetas, entropies = LDA(A, B, K, alpha, gamma)
    print(perplexity)
    I = 20
    ...
    plt.plot(np.arange(50), thetas.T)
    plt.xlim(0, 50)
    plt.xticks([0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48])
    plt.xlabel('Iteration')
    plt.ylabel('Mixing proportion')
    plt.title('Random Seed: 0')
    plt.show()

    plt.plot(np.arange(50), entropies.T)
    plt.xlim(0, 50)
    plt.xticks([0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48])
    plt.xlabel('Iteration')
