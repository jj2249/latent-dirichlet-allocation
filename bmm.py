...
def BMM(A, B, K, alpha, gamma):
    ...
    num_iters_gibbs = 50
    thetas = np.zeros((K, num_iters_gibbs))
    # Perform Gibbs sampling through all documents and words
    for iter in range(num_iters_gibbs):
        thetas[:, iter] = (sk_docs.flatten()+alpha)/np.sum(sk_docs.flatten()+alpha)
        for d in range(D):
...
if __name__ == '__main__':
    ...
    perplexity, swk, thetas = BMM(A, B, K, alpha, gamma)
    print(perplexity)
    I = 20
